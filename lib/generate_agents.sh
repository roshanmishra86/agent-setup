...
process_conditionals() {
    ...
    # Regex patterns using [{}] character classes (ERE treats bare {} as special)
    local re_open='^[[:space:]]*[{][{]#([A-Z0-9_]+)[}][}][[:space:]]*$'
    local re_close='^[[:space:]]*[{][{]/([A-Z0-9_]+)[}][}][[:space:]]*$'
    local re_inline='[{][{]#([A-Z0-9_]+)[}][}]([^{]*)[{][{]/([A-Z0-9_]+)[}][}]'

    while IFS= read -r line; do
        # Check for standalone opening tag on its own line
        ...
        # Check for standalone closing tag
        ...
        # If inside a disabled block, skip the line
        if [ -n "$skip_flag" ]; then
            continue
        fi

        # Handle inline conditionals: {{#FLAG}}content{{/FLAG}} on same line
        if [[ "$line" == *'{{#'* ]]; then
...
# Replace simple {{PLACEHOLDER}} variables in content
substitute_vars() {
    local content="$1"
